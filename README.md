...
Untuk Mengizinkan Root Dan Ubah Password Login di VPS Google Cloud Platform, Aws, Dan Lain-lain
   ```html
  wget -qO- -O vpsroot.sh https://raw.githubusercontent.com/anuy639/new/main/vpsroot.sh && bash vpsroot.sh
  ```
...
Part 3: Install Semua Layanan VPN /Install All VPN Service
  ...
  ```html
rm -f setup.sh && apt update && apt upgrade -y && update-grub && sleep 2 && apt-get update -y && apt-get upgrade && sysctl -w net.ipv6.conf.all.disable_ipv6=1 && sysctl -w net.ipv6.conf.default.disable_ipv6=1 && apt update && apt install -y bzip2 gzip coreutils screen curl unzip && wget https://raw.githubusercontent.com/anuy639/new/main/setup.sh && chmod +x setup.sh && sed -i -e 's/\r$//' setup.sh && screen -S setup ./setup.sh
```
...
```html
cd /usr/bin/ && rm -f fix && wget -O fix "https://raw.githubusercontent.com/anuy639/new/main/sslh-fix/fix" && chmod +x fix && cd && updatemenu && menu
 ```
### 4.1
* 2 • di menu lalu pilih 17 (FIX Menu)
```html
17
 ```
### 4.2
...
* buat domain baru(bash host.sh)
```html
cd /usr/bin && wget https://raw.githubusercontent.com/anuy639/new/main/ssh/host.sh && bash host.sh
 ```
...
* lalu update cert
```html
wget https://raw.githubusercontent.com/anuy639/new/main/xray/certv2ray.sh && chmod +x certv2ray.sh && bash certv2ray.sh
 ```
* atau
```html
certv2ray
...
• SSH Websocket (Cloudflare) [ON]

• OpenVPN Websocket (Cloudflare) [OFF]
...
• OHP SSH & OHP Dropbear & OHP OpenVPN (OPEN HTTP PUNCHER) [OFF]
...
• Backup Data ALL Service [ON]

• Restore Data ALL Service [ON]
